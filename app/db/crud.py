...
def get_user_usage_top10(db: Session, start: datetime, end: datetime) -> UserUsageTop10Response:
    nodes = {}
    usages = {}
    ...
    cond = and_(NodeUserUsage.created_at >= start,
                NodeUserUsage.created_at <= end)
    for v in db.query(NodeUserUsage).filter(cond):
        try:
            item = usages.get(v.user_id)
            if not item:
                item = {
                ...
                usages[v.user_id] = item

                for node_id in nodes.keys():
                    item["usages"][node_id] = 0

            item["total"] += v.used_traffic
            item["usages"][v.node_id or 0] += v.used_traffic
        except KeyError:
            pass

    usages = list(usages.values())
    usages.sort(key=lambda v: -v["total"])
    usages = usages[0:9]

    top10_usages = UserUsageTop10Response(usages=[], users=[])

    for v in usages:
        top10_usages.users.append(v["username"])
        if len(top10_usages.users) >= 10:
            break

    for node_id, node_name in nodes.items():
        top10 = UserUsageTop10Response.Item(name=node_name, data=[])
        top10_usages.usages.append(top10)
        for v in usages:
            top10.data.append(v["usages"][node_id])
            if len(top10.data) >= 10:
                break 
...
def reset_all_data_usage(db: Session, admin: Optional[Admin] = None):
    for dbuser in db.query(User).all():
        dbuser.used_traffic = 0
        db.add(dbuser)    

    for dbnode in db.query(Node).all():
        dbnode.uplink = 0
        dbnode.downlink = 0
        db.add(dbnode)

    for dbsys in db.query(System).all():
        dbsys.uplink = 0
        dbsys.downlink = 0
        db.add(dbsys)

    db.query(NodeUsage).delete()
    db.query(NodeUserUsage).delete()
    db.query(UserUsageResetLogs).delete()
...
ClashLinkSortingOptions = Enum('ClashLinkSortingOptions', {
    'id': ClashLink.id.asc(),
    'modified_at': ClashLink.modified_at.asc(),
    'tag': ClashLink.tag.asc(),
    'name': ClashLink.name.asc(),
    'url': ClashLink.url.asc(),
    '-id': ClashLink.id.desc(),
    '-modified_at': ClashLink.modified_at.desc(),
    '-tag': ClashLink.tag.desc(),
    '-name': ClashLink.name.desc(),
    '-url': ClashLink.url.desc(),
})

def get_clash_link(db: Session, id: int) -> ClashLink:
    return db.query(ClashLink).filter(ClashLink.id == id).first()

def create_clash_link(db: Session, link: ClashLinkCreate):
    dblink = ClashLink(
        name=link.name,
        tag=link.tag,
        prefix=link.prefix,
...
def update_clash_link(db: Session, dblink: ClashLink, modify: ClashLinkCreate):
    old_tag = dblink.tag
    new_tag = modify.tag

    dblink.name = modify.name
    dblink.tag = modify.tag
    dblink.prefix = modify.prefix
    dblink.url = modify.url
    dblink.content = ""
    dblink.modified_at = datetime.utcnow()
    db.commit()
    db.refresh(dblink)
...
def remove_clash_link(db: Session, dblink: ClashLink):
    old_tag = dblink.tag
    new_tag = ""
...
def get_clash_links(db: Session,
              offset: Optional[int] = None,
              limit: Optional[int] = None,
              search: Optional[str] = None,
              sort: Optional[List[ClashLinkSortingOptions]] = None,
            ) -> Tuple[List[ClashLink], int]:
    query = db.query(ClashLink)

    if search:
        query = query.filter(or_(ClashLink.name.ilike(f'%{search}%'),
                                 ClashLink.tag.ilike(f'%{search}%')))
