...
from app.models.clash import (
    ...
    ClashProxyBriefResponse,
    ClashProxyBriefsResponse,
    ClashProxyCreate,
    ClashProxyGroupCreate,
    ClashProxyGroupResponse,
    ClashProxyGroupsResponse,
    ClashProxyInboundResponse,
    ClashProxyInboundsResponse,
    ClashProxyResponse,
    ClashProxyTagResponse,
    ClashProxyTagsResponse,
    ClashRuleCreate,
    ClashRuleResponse,
    ClashRulesetCreate,
    ClashRulesetResponse,
    ClashRulesetsResponse,
    ClashRulesResponse,
    ClashSettingCreate,
    ClashSettingResponse,
    ClashSettingsResponse
)
...
@router.post("/clash/link", tags=['Clash'], response_model=ClashLinkResponse)
def add_clash_link(created: ClashLinkCreate,
             db: Session = Depends(get_db),
             admin: Admin = Depends(Admin.get_current)):
    try:
        dblink = crud.create_clash_link(db, created)
    except sqlalchemy.exc.IntegrityError:
        raise HTTPException409("LinkExists", "Link already exists")
    
    logger.info(f"New link \"{dblink.name}\" added")
    return dblink

@router.put("/clash/link/{id}", tags=['Clash'], response_model=ClashLinkResponse)
def modify_clash_link(id: int,
                modified: ClashLinkCreate,
                db: Session = Depends(get_db),
                admin: Admin = Depends(Admin.get_current)):

    dblink = crud.get_clash_link(db, id)
    if not dblink:
        raise HTTPException404("Link not found")
    
    crud.update_clash_link(db, dblink, modified)

    logger.info(f"Link \"{dblink.id}\" modified")

    return dblink

@router.delete("/clash/link/{id}", tags=['Clash'])
def delete_clash_link(id: int,
                db: Session = Depends(get_db),
                admin: Admin = Depends(Admin.get_current)):
    ...
    crud.remove_clash_link(db, dblink)

    logger.info(f"Link \"{dblink.id}\" deleted")

    return {}

@router.get("/clash/links", tags=['Clash'], response_model=ClashLinksResponse)
def get_clash_links(offset: int = None,
              limit: int = None,
              search: str = None,
              sort: str = None,
              db: Session = Depends(get_db),
    ...
    if sort is not None:
        ...
        for opt in opts:
            try:
                sort.append(crud.ClashLinkSortingOptions[opt])
            except KeyError:
                raise HTTPException400("InvalidSortOption", f'"{opt}" is not a valid sort option')

    data, count = crud.get_clash_links(db=db, 
                                  offset=offset,
                                  limit=limit,
                                  search=search,
                                  sort=sort)
...
def get_clash_proxy_tags(db: Session = Depends(get_db),
    ...
    for link in crud.get_clash_links(db)[0]:
        add_tag(link, "<Link>")
...
def generate_subscription(db: Session, dbuser: User, user_agent: str):
    ...
    if option.is_v2ray:
        ...
        content = generate_subscription_with_rules(db=db, dbuser=dbuser, user_agent=user_agent, option=option)
...
class ClashConfig:
    class Option:
        ...
        def __str__(self) -> str:
            strs = [
                ...
                f"hysteria2={self.support_hysteria2}",
    ...
    def __init__(self, db: Session, dbuser: User, user_agent: str, option: Option) -> None:
        self.option = option
        self.db = db
        self.dbuser = dbuser
        ...
        self.links: list[ClashConfig.Link] = []
        self.links_by_id: Dict[int, ClashConfig.Link] = {}
    ...
    def init_proxies(self):
        ...
        for obj in crud.get_clash_proxies(db=self.db)[0]:
            ...
            if idx >= 0 or (len(tags) == 0 and obj.builtin):
                proxy = ClashConfig.Proxy.model_validate(obj)
        ...
        for obj in crud.get_clash_links(db=self.db)[0]:
            try:
                idx = tags.index(obj.tag)
            except Exception:
                idx = -1
            if idx >= 0 or (len(tags) == 0 and obj.builtin):
                link = ClashConfig.Link.model_validate(obj)
                link.exported = False
                link.tag_index = idx
                self.links_by_id[link.id] = link
                self.links.append(link)
        self.links.sort(key=lambda v: v.tag_index)
        ...
        for obj in crud.get_clash_proxy_groups(db=self.db)[0]:
            ...
            if obj.builtin or idx >= 0:
                proxy_group = ClashConfig.ProxyGroup.model_validate(obj)
        ...
        self.builtin_proxy_groups["SPEEDTEST"] = self.proxy_groups_by_id.get(5)
    ...
    def init_rulesets(self):
        ...
        for ruleset in crud.get_clash_rulesets(self.db):
            self.rulesets.append(ClashConfig.Ruleset.model_validate(ruleset))

            if ruleset.policy == "REJECT":
                sorted0[ruleset.name] = 0

            for obj in ruleset.rules:
                self.rules.append(ClashConfig.Rule.model_validate(obj))
    ...
    def write_proxy_group_setting(self, obj: ProxyGroup, proxy_group: dict):
        if obj.type == "url-test":
            proxy_group["tolerance"] = R(obj.settings, "url_test.tolerance")
            proxy_group["lazy"] = R(obj.settings, "url_test.lazy")
            proxy_group["url"] = R(obj.settings, "url_test.url")
            proxy_group["interval"] = R(obj.settings, "url_test.interval")
        elif obj.type == "fallback":
            proxy_group["url"] = R(obj.settings, "fallback.url")
            proxy_group["interval"] = R(obj.settings, "fallback.interval")
        elif obj.type == "load-balance":
            proxy_group["url"] = R(obj.settings, "load_balance.url")
            proxy_group["interval"] = R(obj.settings, "load_balance.interval")
            proxy_group["strategy"] = R(obj.settings, "load_balance.strategy")
        elif obj.type == "select":
            proxy_group["disable-udp"] = R(obj.settings, "select.disable_udp")
            proxy_group["filter"] = R(obj.settings, "select.filter")
    ...
    def write_proxies(self):
        ...
        for obj in self.proxy_groups:
            ...
            self.config["proxy-groups"].append(proxy_group)
            self.write_proxy_group_setting(obj, proxy_group)
        ...
        for link in self.links:
            try:
                content = ""
                data = None
                modified_at = datetime.fromisoformat(link.modified_at.isoformat() + "+00:00").timestamp()
                now = datetime.now(timezone.utc).timestamp()
                if not link.content or len(link.content) == 0 or modified_at < now - 3600:
                    response = requests.get(link.url, headers={'User-Agent': self.user_agent})
                    if response.status_code == 200:
                        content = response.text
                        data = yaml.safe_load(content)
                        db = next(get_db())
                        dblink = crud.get_clash_link(db, link.id)
                        dblink.content = content
                        dblink.modified_at = datetime.now(timezone.utc)
                        db.commit()
                        print(f'request {link.url} success')
                    else:
                        print(f'request {link.url} failed')
                else:
                    data = yaml.safe_load(link.content)

                if not data:
                    continue

                for group in data["proxy-groups"]:
                    group["name"] = f"{link.prefix}|{group['name']}"
                    proxies = []
                    for name in group["proxies"]:
                        if name != "DIRECT" and name != "REJECT" and name != "MATCH":
                            proxies.append(f"{link.prefix}|{name}")
                        else:
                            proxies.append(name)
                    group["proxies"] = proxies
                    node_names.append(group["name"])
                    self.config["proxy-groups"].append(group)

                for proxy in data["proxies"]:
                    proxy["name"] = f"{link.prefix}|{proxy['name']}"
                    node_names.append(proxy["name"])
        ...
        builtins = [self.builtin_proxy_groups["PROXY"], 
                   ...
                   self.builtin_proxy_groups["SPEEDTEST"]]
        builtins.reverse()
        for obj in builtins:
            proxies = []
            for id in obj.proxies.split(','):
            ...
            self.config["proxy-groups"].insert(0, proxy_group)
            self.write_proxy_group_setting(obj, proxy_group)

        for ruleset in self.rulesets:
            if ruleset.builtin:
                continue
            rs_nodes = ["DIRECT", "PROXY", "REJECT", "SPEEDTEST"]
            ...
            rs_nodes[3] = self.builtin_proxy_groups[rs_nodes[3]].name
            rs_nodes += node_names
            self.config["proxy-groups"].append({
                "name": ruleset.name,
                "type": "select",
...
def generate_subscription_with_rules(
    ...
    clash = ClashConfig(db=db, dbuser=dbuser, user_agent=user_agent, option=option)
    clash.init_proxies()
    clash.init_rulesets()
    clash.write_proxies()
