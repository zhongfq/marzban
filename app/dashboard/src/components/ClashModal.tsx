...
import { ClashLinkDialog } from "./ClashLinkDialog";
          ...
          <ModalHeader pt={6}>
            <HStack gap={2}>
              ...
              <TabList>
                ...
                <Tab fontSize="lg">{t("clash.links")}</Tab>
...
const Setting: FC<BoxProps> = () => {
  ...
  const makeIcon = (name: string) => {
    return joinPaths([import.meta.env.BASE_URL, `/statics/images/${name}.png`]);
  };
...
const setLinkSearchField = debounce((search: string) => {
  useClash.getState().onLinkFilterChange({
    ...useClash.getState().linkFilter,
    offset: 0,
    search,
  });
}, 300);

const Links: FC<StackProps> = () => {
  const { isEditingSubscription } = useDashboard();
  const { t } = useTranslation();
  const {
    linkFilter,
  ...
  const onChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setSearch(e.target.value);
    setLinkSearchField(e.target.value);
  };
  const clear = () => {
    setSearch("");
    onLinkFilterChange({
      ...
      <Box overflowX="auto" w="full">
        <Table orientation="vertical">
          <Thead zIndex="docked" position="relative">
            <Tr>
              ...
              >
                <HStack>
                  <span>{t("clash.name")}</span>
                  <Sort sort={linkFilter.sort} column="name" />
                </HStack>
              </Th>
              <Th
                minW="140px"
              ...
              >
                <HStack>
                  <span>{t("clash.tag")}</span>
                  <Sort sort={linkFilter.sort} column="tag" />
                </HStack>
              </Th>
              <Th
                minW="250px"
                cursor={"pointer"}
                onClick={handleSort.bind(null, "modified_at")}
              >
                <HStack>
                  <span>{t("clash.modified_at")}</span>
                  <Sort sort={linkFilter.sort} column="modified_at" />
                </HStack>
              </Th>
              <Th
                minW="140px"
                cursor={"pointer"}
                onClick={handleSort.bind(null, "url")}
              >
                <HStack>
                  <span>{t("clash.url")}</span>
                  <Sort sort={linkFilter.sort} column="url" />
                </HStack>
              </Th>
            </Tr>
          </Thead>
          <Tbody>
            {links?.map((value) => {
              return (
                <Tr key={value.name} onClick={() => onEditingLink(value)}>
                  <Td>{value.name}</Td>
                  <Td>{value.tag}</Td>
        ...
        <Pagination
          ...
          total={totalLinks}
          limit={linkFilter.limit}
          offset={linkFilter.offset}
        />
      </Box>
    </VStack>
  );
